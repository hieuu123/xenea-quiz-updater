...
TARGET_H2_TEXT = "Xenea Wallet Daily Quiz Today for December 4, 2025"
CHECK_ANSWER = "B) Via official updater."
...
def scrape_quiz_site2():
    url = "https://www.quiknotes.in/xenea-wallet-daily-quiz-answer-5-december-2025/"
    print(f"[+] Scraping quiz from {url}")
    r = requests.get(url, timeout=15, headers={"User-Agent": "Mozilla/5.0"})
    r.raise_for_status()
    soup = BeautifulSoup(r.text, "html.parser")
