...
POST_ID = 298882  # ID bài muốn update
TARGET_H2_TEXT = "Xenea Wallet Daily Quiz Today’s Answer – October 29, 2025"
CHECK_ANSWER = "B) An EVM Layer-1 with integrated decentralized storage."
...
def scrape_quiz_site2():
    url = "https://www.quiknotes.in/xenea-wallet-daily-quiz-answer-29-october-2025/"
    print(f"[+] Scraping quiz from {url}")
    r = requests.get(url, timeout=15, headers={"User-Agent": "Mozilla/5.0"})
    r.raise_for_status()
    soup = BeautifulSoup(r.text, "html.parser")
