...
POST_ID = 301017  # ID bài muốn update
TARGET_H2_TEXT = "Xenea Wallet Daily Quiz Today’s Answer – November 7, 2025"
CHECK_ANSWER = "A) They produce blocks every minute."
...
def scrape_quiz_site2():
    url = "https://www.quiknotes.in/xenea-wallet-daily-quiz-answer-7-november-2025/"
    print(f"[+] Scraping quiz from {url}")
    r = requests.get(url, timeout=15, headers={"User-Agent": "Mozilla/5.0"})
    r.raise_for_status()
    soup = BeautifulSoup(r.text, "html.parser")
