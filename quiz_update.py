...
TARGET_H2_TEXT = "Xenea Wallet Daily Quiz Today’s Answer for November 26, 2025"
CHECK_ANSWER = "D) Verify source authenticity."

# ngày find & replace
OLD_DATE = "November 26"
NEW_DATE = "November 27"
...
def scrape_quiz_site2():
    url = "https://www.quiknotes.in/xenea-wallet-daily-quiz-answer-25-november-2025/"
    print(f"[+] Scraping quiz from {url}")
    r = requests.get(url, timeout=15, headers={"User-Agent": "Mozilla/5.0"})
    r.raise_for_status()
    soup = BeautifulSoup(r.text, "html.parser")
